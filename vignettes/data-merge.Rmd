...
the term "merge" to combine datasets where two functions are offered `data_merge_module` and `data_merge_srv`. Depending on the case scenario, one or the other shall be used. 

When no pre-processing of the `data_extract` list is required, `data_merge_module` function is used as a wrapper that combines `data_extract_multiple_srv()` and `data_merge_srv()` to read the data and the `data_extract_spec`'s list and apply the merging.
...
In the coming sections, we will show examples of both case scenarios.

### `data_merge_module`

`data_merge_module` solely replaces passing `data_extract_multiple_srv` output to `data_merge_srv` so all you would need is 
a list of `data_extract_spec` objects for the `data_extract` argument and a `FilteredData` object for the `datasets` argument.
...
In the scenario above, if the user de-selects the `ADTTE` variable, the merging between `ADTTE` and `ADSL` would still take place even though `ADTTE` is not used or needed here. Here, the developer might update the `selector_list` input reactively so that it gets updated based on conditions set by the developer. Below, we reuse the input from above and we update the `app` server so that the `adtte_extract` is removed from the `selector_list` input when no `ADTTE` variable is selected and the `reactive_selector_list` is passed to `data_merge_srv`:
...
1) `selector_list`: output of `data_extract_multiple_srv`
...
General datasets do not share the same relationships as CDISC datasets thus these relationships shall be specified by the `join_keys` functions. For more information, please refer to the `Join Keys` [vignette](https://insightsengineering.github.io/teal/join-keys).

The data merge module respects the relationships given by the user and in the case of multiple datasets to merge, the order is specified by the order of elements in the `data_extract` argument of the `data_merge_module` function. Merging groups of datasets with complex relationships can quickly become challenging to specify so please contact the NEST team to discuss your use case.
