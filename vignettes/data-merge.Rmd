...
merge_module <- function(id, datasets, data_extracts, join_keys) {
  moduleServer(id, function(input, output, session) {
    ...
    teal.code::chunks_push(str2lang("ADSL <- scda::synthetic_cdisc_data('latest')$adsl"), chunks = chunks_h)
    teal.code::chunks_push(str2lang("ADTTE <- scda::synthetic_cdisc_data('latest')$adtte"), chunks = chunks_h)
...
# create a list of join_keys
join_keys <- list(
  ADSL = list(
    ADSL = c(STUDYID = "STUDYID", USUBJID = "USUBJID"),
    ADTTE = c(STUDYID = "STUDYID", USUBJID = "USUBJID")
  ),
  ADTTE = list(
    ADSL = c(STUDYID = "STUDYID", USUBJID = "USUBJID"),
    ADTTE = c(STUDYID = "STUDYID", USUBJID = "USUBJID", PARAMCD = "PARAMCD")
  )
)
```
...
```{r eval=FALSE}
shinyApp(
  ui = fluidPage(merge_ui("data_merge", data_extracts, join_keys)),
  server = function(input, output, session) {
    merge_module("data_merge", datasets, data_extracts, join_keys)
  }
)
