...
Combining datasets is an essential step when working with modules that involve more than one dataset. Within the context of `teal`, we use the term "merge" to refer to the process of combining datasets. Two functions are provided for this purpose: `merge_expression_module` and `merge_expression_srv`. The choice between them depends on the specific scenario.

When no processing of the `data_extract` list is required, you can use the `merge_expression_module` function. This function reads the data and the list of `data_extract_spec` objects and applies the merging. Essentially, it serves as a wrapper that combines `data_extract_multiple_srv()` and `merge_expression_srv()`. Further details are provided below. In scenarios where additional processing of the `data_extract` list is necessary, `merge_expression_srv()` can be used along with `data_extract_multiple_srv()` or `data_extract_srv()` to customize the `selector_list` input.
...
General datasets do not have the same relationships as `CDISC` datasets, so these relationships must be specified using the `join_keys` functions. For more information, please refer to the `Join Keys` [vignette](https://insightsengineering.github.io/teal.data/latest-tag/articles/join-keys).
