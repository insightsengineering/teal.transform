...
The `teal.transform` package provides `merge_expression_srv` which converts `data_extract_srv` into `R` expression to transform data
...
Here, we create the `merge_ui` and the `merge_srv` functions which will be used to create the UI and the server
...
(list of reactive `data.frame` objects) and `join_keys` object. `merge_expression_srv` returns a reactive list containing merge expression and information needed to perform the transformation - see more in `merge_expression_srv` documentation.

#### Example data

`data_extract_srv` module depends on either a list of reactive or non-reactive `data.frame` objects. Here, we show the 
usage of a list of reactive `data.frame` objects as input to `datasets`
...
Finally, we pass `merge_ui` and `merge_srv` to the UI and server component of the `shinyApp`
