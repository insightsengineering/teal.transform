...
#' choices1 <- choices_labeled(names(ADSL), formatable::var_labels(ADSL, fill = FALSE))
...
variable_choices.data.frame <- function(data, subset = NULL, fill = FALSE, key = NULL) { # nolint
  ...
  res <- if ("" %in% subset) {
    choices_labeled(
      ...
      labels = c("", unname(formatable::var_labels(data, fill = fill))),
      subset = subset,
      types = c("", var_types)
    )
  } else {
    choices_labeled(
      choices = names(data),
      labels = unname(formatable::var_labels(data, fill = fill)),
      subset = subset,
      types = var_types
    )
  }
