...
#' choices1 <- choices_labeled(names(ADSL), teal.data::formatters_var_labels(ADSL, fill = FALSE))
...
#'   names(ADSL),
#'   teal.data::formatters_var_labels(ADSL, fill = FALSE),
#'   subset = c("ARMCD", "ARM")
#' )
#' \dontrun{
#' shinyApp(
...
variable_choices.data.frame <- function(data, subset = NULL, fill = TRUE, key = NULL) { # nolint
  ...
  res <- if ("" %in% subset) {
    choices_labeled(
      ...
      labels = c("", unname(teal.data::formatters_var_labels(data, fill = fill))),
      subset = subset,
      types = c("", var_types)
    )
  } else {
    choices_labeled(
      choices = names(data),
      labels = unname(teal.data::formatters_var_labels(data, fill = fill)),
      subset = subset,
      types = var_types
    )
  }
