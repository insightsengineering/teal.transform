...
test_that("names and variables are reported", {
  ...
  d_df <- datasets("df")
  df_upper_variables <- c(d_df, variables(function(x) {
    x == toupper(x)
  }))
...
test_that("update_spec resolves correctly", {
  ...
  data_frames_factors <- c(datasets(is.data.frame), variables(is.factor))
...
test_that("OR specifications resolves correctly", {
  ...
  var_a <- variables("A")
  df_a <- c(datasets(is.data.frame), var_a)
  matrix_a <- c(datasets(is.matrix), var_a)
